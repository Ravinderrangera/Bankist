...
btnLogin.addEventListener("click", function(e) {
    ...
    if (currentAccount?.pin === +inputLoginPin.value) {
        ...
        labelDate.textContent = new Intl.DateTimeFormat(currentAccount.locale, options).format(noW); //Now the date is actually formatted, just like it is ususally formatted in the U.S. with the month first and then the day. And to get these different codes, just goolge "iso language code table".
...
console.log(Number.isNaN(NaN)); //This is offcourse NaN.
...
*/ // 287,460,000,000
const diameter = 287460000000; //its relly hard to read here there are just too many zeros to read here.
...
const priceCents = 34599; //the value after the underscore is represented as cents.
console.log(priceCents);
const transferFee1 = 1500;
...
console.log(438975348925734895734857348957348957349857348957348957348957348957n * 10000000000000000000n);
...
console.log(new Date(259200000)); //(days * hours * min * seconds * miliseconds) And so now we get Jan 04, so that's exactly three days later. So this number that we calculated here can also be calculated in the console so the result is called the timestamp. So call this a timestamp of day number 3 which is this 259200000.
...
console.log(+future2);
const daysPassed = (date1, date2)=>Math.abs((date2 - date1) / 86400000); //1000 converts milliseconds to seconds then times 60 to convert it to minutes, then times 60 to convert it to hours, and then times 24, which converts it to days.
...
const intlExample = new Intl.DateTimeFormat(navigator.language, options2).format(todaysdate);
...
console.log("US:                ", new Intl.NumberFormat("en-US", options).format(number)); //So you see that the number now is formatted using these dividers so these commas as seprators so thta it's easier to read 3,884 thousand and so on.
...
console.log(navigator.language, new Intl.NumberFormat(navigator.language, options).format(number)); //So in UK it is basically the same as in the US.
